...
auto Trie::Put(std::string_view key, T value) const -> Trie {
	...
	std::shared_ptr<TrieNode> node = root_? std::shared_ptr<TrieNode>(std::move(root_->Clone())) : std::make_shared<TrieNode>(); 
	
	for (auto ch : key) {
		node_stack.push(node);
		if (!node->children_.count(ch)) {
...
auto Trie::Remove(std::string_view key) const -> Trie {
	...
	for (auto it = key.rbegin(); it != key.rend(); ++it) {		
		...
		node = node_stack.top();
		node_stack.pop();
		if (node->children_[*it]->children_.empty() and !node->children_[*it]->is_value_node_) {
